...
io.on('connection', (socket) => {
    ...
    socket.on('sendIceCandidateToSignalingServer', ({ iceCandidate, targetUserName, meetingCode }) => {
        const senderClient = connectedSockets.find(client => client.socketId === socket.id);
        const senderUserName = senderClient ? senderClient.userName : null; // Get the username or null if not found

	console.log(`ICE Candidate received from ${senderUserName} for ${targetUserName}`);
        ...
        if (targetClient) {
            ...
            io.to(targetClient.socketId).emit('receiveIceCandidate', {
                iceCandidate,
                userName: senderUserName  // The user who is sending the ICE candidate
            });
